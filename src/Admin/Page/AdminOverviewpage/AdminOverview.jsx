...
export const AdminOverview = () => {
    ...
    const fetchTeachersLength = async () => {
        ...
        try {
            const result = await axios(BASE_URL + "/api/teachers-len", {
                timeout: 20000
            });
    ...
    const fetchStudentsLength = async () => {
        ...
        try {
            if (sessionStorage.getItem('role') === 'Admin') {
                const result = await axios(BASE_URL + "/api/students-len", {
                    timeout: 20000
                });
                setStudentsLen(result.data);
            }
            else if (sessionStorage.getItem('role') === 'Teacher') {
                const result = await axios(BASE_URL + `/api/students-len/${sessionStorage.getItem('id')}`, {
                    timeout: 20000
                });
                setStudentsLen(result.data);
            }
    ...
    const fetchLessonsLength = async () => {
        ...
        try {
            if (sessionStorage.getItem('role') === 'Admin') {
                const result = await axios(BASE_URL + "/api/lessons-len", {
                    timeout: 20000
                });
                setLessonsLen(result.data);
            }
            else if (sessionStorage.getItem('role') === 'Teacher') {
                const result = await axios(BASE_URL + `/api/lessons-len/${sessionStorage.getItem('id')}`, {
                    timeout: 20000
                });
                setLessonsLen(result.data);
            }
